...
export default function ReportsView({ projects, tasks }) {
  const [reportView, setReportView] = useState('tasks')
  ...
  const [reportFilterTaskId, setReportFilterTaskId] = useState(null)
  ...
  const taskRows = useMemo(() => {
    ...
    for (const p of projects) {
      ...
      for (const t of projectTasks) {
        ...
        all.push({
          project: p.title, projectDept: p.departmentId, projectId: p.id,
          task: t.title, taskId: t.id, assignee: t.assignee,
          status: t.status, priority: t.priority, dueDate: t.dueDate,
          deptName: dept?.name || '-', deptIcon: dept?.icon || '', deptColor: dept?.color || '#6b7280',
          subtaskDone: done, subtaskTotal: total, subtaskPct: total ? Math.round(done / total * 100) : 0,
        })
      }
    }
    return all
  ...
  const subtaskRows = useMemo(() => {
    ...
    for (const p of projects) {
      const projectTasks = tasks.filter(t => t.projectId === p.id)
      for (const t of projectTasks) {
        if (reportFilterTaskId && t.id !== reportFilterTaskId) continue
        for (const s of (t.subtasks || [])) {
          const member = getMemberById(s.assignee)
          all.push({
            task: t.title, taskId: t.id,
            project: p.title, projectId: p.id,
            subtask: s.title, subtaskId: s.id,
            assignee: s.assignee, assigneeName: member ? member.name : 'Unassigned',
            assigneeAvatar: member ? member.avatar : '',
            status: s.status, priority: s.priority || t.priority, dueDate: s.dueDate || t.dueDate,
          })
        }
      }
    }
    return all
  }, [projects, tasks, reportFilterTaskId])

  const rows = reportView === 'tasks' ? taskRows : subtaskRows
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter(r => {
        if (reportView === 'tasks') return r.task.toLowerCase().includes(q) || r.project.toLowerCase().includes(q)
        return r.subtask.toLowerCase().includes(q) || r.task.toLowerCase().includes(q) || r.assigneeName.toLowerCase().includes(q)
      })
    }
    result.sort((a, b) => {
      let cmp = 0
      if (reportView === 'tasks') {
        ...
        else if (sortKey === 'priority') { const o = { critical: 0, high: 1, medium: 2, low: 3 }; cmp = (o[a.priority] ?? 99) - (o[b.priority] ?? 99) }
        ...
        else if (sortKey === 'subtask') cmp = a.subtask.localeCompare(b.subtask)
        else if (sortKey === 'assigneeName') cmp = a.assigneeName.localeCompare(b.assigneeName)
        else if (sortKey === 'status') cmp = a.status.localeCompare(b.status)
        else if (sortKey === 'priority') { const o = { critical: 0, high: 1, medium: 2, low: 3 }; cmp = (o[a.priority] ?? 99) - (o[b.priority] ?? 99) }
        else if (sortKey === 'dueDate') cmp = new Date(a.dueDate) - new Date(b.dueDate)
      }
    ...
    return result
  }, [rows, search, sortKey, sortDir, filterProject, filterStatus, filterStatus2, reportView])
  ...
  const exportCSV = () => {
    if (reportView === 'tasks') {
      ...
      const a = document.createElement('a'); a.href = url; a.download = 'task-report.csv'; a.click()
      URL.revokeObjectURL(url)
    } else {
      const headers = ['Task','Subtask','Assignee','Status','Priority','Due Date']
      const csv = [headers.join(',')]
      for (const r of filtered) {
        csv.push([`"${r.task}"`,`"${r.subtask}"`,`"${r.assigneeName}"`,r.status,r.priority,fmt(r.dueDate)].join(','))
      }
      const blob = new Blob([csv.join('\n')], { type: 'text/csv' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a'); a.href = url; a.download = 'subtask-report.csv'; a.click()
      URL.revokeObjectURL(url)
    }
  }
    ...
    <div className="reports-view">
      <div className="reports-toolbar">
        <div className="reports-title-row">
          ...
          <select className="form-select" style={{ width: 120 }} value={reportView} onChange={e => { setReportView(e.target.value); setReportFilterTaskId(null) }}>
            <option value="tasks">Tasks</option>
            <option value="subtasks">Subtasks</option>
          </select>
        </div>
        <div className="reports-summary">
          {reportView === 'subtasks' && reportFilterTaskId ? (
            <span className="reports-summary-item" style={{ fontWeight: 600 }}>
              Subtasks for: {tasks.find(t => t.id === reportFilterTaskId)?.title || ''}
            </span>
          ) : null}
      ...
      <div className="reports-filters">
        <div className="reports-filter-group">
          ...
          <div className="search-box" style={{ width: 180 }}>
            <input type="text" placeholder={reportView === 'tasks' ? 'Search tasks...' : 'Search subtasks...'} value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        </div>
        <div className="reports-filter-group">
        ...
        {reportView === 'tasks' ? (
              ...
              {filtered.map(r => {
                ...
                return (
                  <tr key={r.taskId}>
                    ...
                    <td>
                      <div className="reports-subtask-cell clickable" onClick={() => { setReportView('subtasks'); setReportFilterTaskId(r.taskId) }}>
          ...
          <table className="reports-table">
            <thead>
              <tr>
                <th onClick={() => toggleSort('task')} className="reports-th-sort">Task{sortIcon('task')}</th>
                <th onClick={() => toggleSort('subtask')} className="reports-th-sort">Subtask{sortIcon('subtask')}</th>
                <th onClick={() => toggleSort('assigneeName')} className="reports-th-sort">Assignee{sortIcon('assigneeName')}</th>
                <th onClick={() => toggleSort('status')} className="reports-th-sort">Status{sortIcon('status')}</th>
                <th onClick={() => toggleSort('priority')} className="reports-th-sort">Priority{sortIcon('priority')}</th>
                <th onClick={() => toggleSort('dueDate')} className="reports-th-sort">Due Date{sortIcon('dueDate')}</th>
              </tr>
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={6} className="reports-empty">No subtasks match the current filters.</td></tr>
              )}
              {filtered.map((r, i) => {
                const sInfo = getStatusInfo(r.status)
                const pInfo = getPriorityInfo(r.priority)
                const overdue = r.status !== 'done' && new Date(r.dueDate) < new Date()
                return (
                  <tr key={r.subtaskId + '-' + i}>
                    <td className="reports-cell-task">{r.task}</td>
                    <td>{r.subtask}</td>
                    <td>
                      <span className="reports-assignee">
                        {r.assigneeAvatar && <span className="reports-avatar">{r.assigneeAvatar}</span>}
                        {r.assigneeName}
                      </span>
                    </td>
                    <td><span className="reports-badge reports-badge-status" style={{ background: sInfo.color }}>{sInfo.label}</span></td>
                    <td><span className="reports-badge reports-badge-priority" style={{ background: pInfo.color }}>{pInfo.label}</span></td>
                    <td className={`reports-cell-date ${overdue ? 'overdue' : ''}`}>
                      {fmt(r.dueDate)}
                      {overdue && <span className="reports-overdue-tag">overdue</span>}
                    </td>
